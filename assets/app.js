...
async function analyze(url, strategy = 'mobile') {
  if (!PAGESPEED_API_KEY || PAGESPEED_API_KEY === 'YOUR_PAGESPEED_API_KEY') {
    throw new Error('PageSpeed API key is not configured. Open assets/app.js and replace YOUR_PAGESPEED_API_KEY with your own key.');
  }
  ...
  let response;
  try { response = await fetch(`${PAGESPEED_API_URL}?${params.toString()}`, { headers: { Accept: 'application/json' }, cache: 'no-store' }); }
  catch { throw new Error('The Google PageSpeed Insights API could not be reached. Check your internet connection and API key restrictions.'); }
